...
with st.container(horizontal=True, gap='medium'):
    ...
    with cols[0]:
        st.metric(
            'Max Precipitation',
            f"{max_prec_2015:0.1f}",
            delta=f"{max_prec_2015-max_temp_2014:0.1f}",
            width='content'
        )
    ...
    with cols[1]:
        st.metric(
            'Min Precipitation',
            f"{min_prec_2015:0.1f}",
            delta=f"{min_prec_2015-min_prec_2014:0.1f}",
            width='content'
        )
    ...
    with cols[0]:
        weather_name = full_df['weather'].value_counts().head(1).reset_index()['weather'][0]
    ...
    with cols[1]:
        weather_name = full_df['weather'].value_counts().tail(1).reset_index()['weather'][0]
...
with cols[1].container(border=True, height='stretch'):
    ...
    st.altair_chart(
        ...
        .mark_bar()
        .encode(
            alt.X('date:N', timeUnit='monthdate').title('date'),
