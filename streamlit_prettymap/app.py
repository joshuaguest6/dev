...
contour_width = col1style.slider(
    ...
    0,
    30,
    help='Thickness of contour line surrounding the map.',
    key='contour_width'
)

name_on = col2style.checkbox(
    'Display title',
    help='If checked, adds the selected address as the title. Can be customised below.',
    key='name_on'
)
...
with ex1.expander('Export geometriesas GeoJSON'):
    st.write(f'{df.shape} geoemtries')
    st.download_button(
        label='Download',
...
aoi = get_aoi(address='Hickson road, Sydney', radius=1500, rectangular=False)
df = st_get_osm_geometries(aoi=aoi)